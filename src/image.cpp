...
float unclamp_color_component(unsigned char int_color) {
    return double(int_color) / 255.0;
}
...
Image* Image::LoadBMP(const char *filename)
{
    ...
    Vector3f* color_map = new Vector3f[width * height];
    if (line == NULL)
    {
        fprintf(stderr, "Inadequate memory.\n");
        return 0;
    }
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        {
            int c_map_index = (width * i + j);
            double r = unclamp_color_component(line[3*j+2]);
            double g = unclamp_color_component(line[3*j+1]);
            double b = unclamp_color_component(line[3*j]);
            color_map[c_map_index] = Vector3f(r, g, b);
        }
    }
    ...
    return new Image(width, height, color_map);
