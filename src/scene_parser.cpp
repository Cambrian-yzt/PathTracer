...
Sphere *SceneParser::parseSphere() {
    ...
    if (!strcmp(token, "motion")) {
        motion = readVector3f();
        getToken(token);
    }
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Sphere(center, radius, current_material, motion);
}
...
Rectangle *SceneParser::parseRectangle() {
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    Rectangle *nr = new Rectangle(pnt, x, y, current_material);
    return nr;
}
...
Mesh *SceneParser::parseTriangleMesh() {
    ...
    Mesh *answer = new Mesh(filename, current_material, smooth);
